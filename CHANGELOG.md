...
### IPC 포인터 파일 — 앱 재시작 후 훅/브릿지 생존

- IPC 디렉토리는 pid 기반이라 앱 재시작 시 바뀌는데, reattach된 세션의 실행 중인
  CLI 프로세스는 생성 시점 env를 물고 있어 훅 이벤트와 브릿지 요청이 죽은
  디렉토리로 가던 문제 수정
- `~/.mulaude/ipc-current` 포인터 파일에 현재 IPC 경로 기록 — hook 스크립트와
  브릿지 CLI가 env 경로 소실 시 포인터로 재해석
- 부수 효과: **앱 재시작 후 reattach된 세션의 훅 기반 상태 감지도 복구됨**
